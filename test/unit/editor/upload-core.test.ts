...
describe('Editor upload core post', () => {
    ...
    test('能够发送简单的post请求，处理返回的json字符串', done => {
        const mockObject = jest
            .fn()
            .mockImplementation(() =>
                mockXHR({ status: 200, res: JSON.stringify({ data: ['url1'], errno: 0 }) })
            )
    ...
    test('能够发送简单的post请求，处理返回的json对象', done => {
        const mockObject = jest
            .fn()
            .mockImplementation(() => mockXHR({ status: 200, res: { data: ['url1'], errno: 0 } }))
    ...
    test('发送请求失败后会有错误回调', done => {
        const mockObject = jest
            .fn()
            .mockImplementation(() =>
                mockXHR({ status: 500, res: JSON.stringify({ data: 'error', errno: 1 }) })
            )
    ...
    test('发送请求能够监听进度变化', done => {
        const mockObject = jest
            .fn()
            .mockImplementation(() =>
                mockXHR({ status: 200, res: JSON.stringify({ data: ['url2'], errno: 0 }) })
            )
    ...
    test('发送请求成功返回的结果如果不是json字符串，会通过fail回调处理', done => {
        const mockObject = jest
            .fn()
            .mockImplementation(() => mockXHR({ status: 200, res: '{test: 123}' }))
